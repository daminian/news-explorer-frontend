...
function NewsCard({ card, handleClick, handleDeleteClick, loggedIn, image, date, title, text, source, keyword, link }) {
    ...
    const [isArticleSave, setArticleSave] = useState(false)
        ...
        <figure className="card">
            <img className="card__photo" src={image} alt="Фотография статьи" />
            {
                ...
                <div className="card__tooltip">
                    <button className={`card__saved card__saved_${isArticleSave && loggedIn ? "active" : "hidden"} 
                card__saved_${card && !loggedIn && "not-loggedIn"}`}
